...
int ss_twr_initiator(void)
{
    ...
    while (1)
    {
    	//test_run_info((uint8_t*)"Start frame");
        ...
        if (status_reg & DWT_INT_RXFCG_BIT_MASK)
        {
            ...
            if (frame_len <= sizeof(rx_buffer))
            {
                ...
                //snprintf(buffer_text, sizeof(buffer_text), "received: %s", rx_buffer);
                //test_run_info((unsigned char *)buffer_text);
                ...
                if (memcmp(rx_buffer, rx_resp_msg, ALL_MSG_COMMON_LEN) == 0)
                {
                    ...
                    last = curr;
                    curr = HAL_GetTick();
                    snprintf(buffer_text, sizeof(buffer_text), "DIST: %3.2f m sent:%3ld\r", distance, curr-last);
                    test_run_info((unsigned char *)buffer_text);
                }
            }
        }
